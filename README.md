...
- Developments: lock and target acquisition sounds

# GeoFS Missile System v2
Works in conjunction with the Target Tracker (and not without). If MA is on and an aircraft is locked (red diamond), pressing full stop "." (period) will trigger the release of a missile, which will track the aircraft. Upon contact an explosion will be seen with smoke lasting 12 seconds and a permanent black scorch mark (up to 200 scorch marks). 
- Known bugs: 
